...
{
    public Powers powersInstance; // Referência para a instância de Powers
    ...
    [Header("Air Jump")]
    [Header("Air Power Gravity")]
    public float AirgravityStrength;
    public float AirGravityScale;

    [Space(5)]
    public float AirFallGravityMult;
    public float AirMaxFallSpeed;
    public float AirFastFallGravityMult;
    public float AirMaxFastFallSpeed;

    [Space(5)]

    [Header("Air Power Jumps")]
    public float AirJumpHeight; //Height of the player's jump
    public float AirJumpTimeToApex; //Time between applying the jump force and reaching the desired jump height. These values also control the player's gravity and jump force.
    [HideInInspector] public float AirJumpForce; //The actual force applied (upwards) to the player when they jump.

    public float AirJumpCutGravityMult; //Multiplier to increase gravity if the player releases thje jump button while still jumping
    [Range(0f, 1)] public float AirJumpHangGravityMult; //Reduces gravity while close to the apex (desired max height) of the jump
    public float AirJumpHangTimeThreshold; //Speeds (close to 0) where the player will experience extra "jump hang". The player's velocity.y is closest to 0 at the jump's apex (think of the gradient of a parabola or quadratic function)
    [Space(0.5f)]
    public float AirJumpHangAccelerationMult;
    public float AirjumpHangMaxSpeedMult;
	...
	[Header("RunIceAndStickyPlatforms")]
	...
	{
		if (powersInstance != null && powersInstance.isAirPlayer)
		{
            //Calculate gravity strength using the formula (gravity = 2 * jumpHeight / timeToJumpApex^2) 
            AirgravityStrength = -(2 * AirJumpHeight) / (AirJumpTimeToApex * AirJumpTimeToApex);

            //Calculate the rigidbody's gravity scale (ie: gravity strength relative to unity's gravity value, see project settings/Physics2D)
            AirGravityScale = AirgravityStrength / Physics2D.gravity.y;

            //Calculate jumpForce using the formula (initialJumpVelocity = gravity * timeToJumpApex)
            AirJumpForce = Mathf.Abs(AirgravityStrength) * AirJumpTimeToApex;
