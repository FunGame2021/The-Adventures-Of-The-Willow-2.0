...
{
    public static playLevelTest instance;
    ...
    private Transform decor2Container;
        ...
        if (File.Exists(loadPath))
        {
            ...
            if (levelDataWrapper != null)
            {
                ...
                if (sectorData != null)
                {
                    ...
                    foreach (GameObject sectorObject in SectorList)
                    {
                        Debug.Log(sectorObject.name);
                        if (sectorObject.name != sectorName)
                        {
                            //se existir na cena desativa os diferentes
                            sectorObject.SetActive(false);
                        }
                        if(sectorObject.name == sectorName)
                        {
                            //se existir na cena ativa os iguais
                            sectorObject.SetActive(true);
                        }

                        if(sectorObject.name == null)
                        { 
                            //instancia ativa ele e desativa os outros
                            GameObject newsectorObject = Instantiate(SectorPrefab, new Vector3(0, 0, 0), Quaternion.identity);
                            newsectorObject.name = sectorName;
                            SectorList.Add(newsectorObject);

                            SectorThings sectorThings = sectorObject.GetComponent<SectorThings>();
                            decorContainer = sectorThings.decorContainer;
                            decor2Container = sectorThings.decor2Container;
                            objectsContainer = sectorThings.objectsContainer;
                            enemyContainer = sectorThings.enemyContainer;
                            backgroundLocal = sectorThings.backgroundLocal;
                            TimeWeatherLocal = sectorThings.TimeWeatherLocal;
                            GameObjectsContainer = sectorThings.GameObjectsContainer;
                            PlayerPos = sectorThings.PlayerPos;
                            tilemapGrid = sectorThings.tilemapGrid;
                            ...
                            volumeNameTimeWeather = timeWeatherName;
                            ...
                            {
                                // Instancie o novo prefab de fundo no local com base no offset
                                Vector3 spawnPosition = new Vector3(backgroundLocal.position.x, backgroundOffset, backgroundLocal.position.z);
