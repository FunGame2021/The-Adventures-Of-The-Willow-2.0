...
public class ResetSettings : MonoBehaviour
{
    ...
    private void DeleteLocalizationSettings()
    {
        string filePath = Application.persistentDataPath + "/Configurations/" + "LocalizationSettings.json";
        if (File.Exists(filePath))
        {
            File.Delete(filePath);
        }
